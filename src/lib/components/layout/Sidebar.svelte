...
<div class="w-[200px] border-r border-border flex flex-col bg-card shrink-0">
  ...
  <nav class="flex-1 p-2">
    {#each items as item}
      ...
      >
        ...
        {#if item.id === "mods"}
          <span class="ml-auto text-[11px] tabular-nums {navStore.active === item.id ? 'text-accent-foreground/70' : 'text-muted-foreground/60'}">
  ...
  <div class="px-4 py-3 border-t border-border">
    <p class="text-[10px] text-muted-foreground/50 leading-relaxed">Jeode v5.3.2.3</p>
    <p class="text-[10px] text-muted-foreground/50 leading-relaxed">Game v5.3.2</p>
  </div>
</div>
