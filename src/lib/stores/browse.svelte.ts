...
const PER_PAGE = 15;
const LAST_BIN_UPDATE = 1728360000; // 4.5.0 - October 9(-1) 2024
...
class BrowseStore {
  ...
  async install(
    ...
    // Warn on potentially outdated bins
    // TODO: This should probably be handled in Rust for all mods.
    if (mod.date_updated < LAST_BIN_UPDATE && !ignoreOutdated) {
      return {
        installed: [],
        total: 0,
        error: "outdated",
