...
class SettingsStore {
    ...
    async updateCantus<K extends keyof CantusSettings>(key: K, value: CantusSettings[K]) {
        this.cantus = { ...this.cantus, [key]: value };
        if (key === "theme") applyTheme(value as "dark" | "light");
        await this.saveCantus();
    }

    async updateJeode<K extends keyof JeodeSettings>(key: K, value: JeodeSettings[K]) {
